...
# Train an "xgbTree" model with caret.
xgbtree_fit <- caret::train(data = subset(train_caret, select = c("target", selection)),
                    target ~ .,
                    method = "xgbTree",
                    metric = "ROC",
                    trControl = trControl,
...
# Visualization of the most important features.
vip(xgbtree_fit, num_features = 15) + ggtitle("Variable importance")

# Plot tuning.
ggplot(xgbtree_fit)

# Predicting on test_set.
xgbtree_fit_pred <- predict(xgbtree_fit, test, type = "prob")
